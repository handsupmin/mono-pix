...
基于 [`fast-pixelizer`](https://github.com/handsupmin/fast-pixelizer) 驱动。

**输入质量说明**

ChatGPT 生成的“像素画风”图片，通常不是很理想的 Snap 输入。它看起来像像素画，但原图本身往往已经带有格子宽高不均、边界发虚、轴线轻微歪斜等问题。Snap 可以把结果重新规整，但如果原始格子一开始就不一致，就很难保证完美修复。

如果你是专门为了 Snap 去生成新素材，更推荐使用 Nano Banana，或者任何从一开始就能更稳定保持正方形低分辨率网格的生成流程。
