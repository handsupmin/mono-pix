...
## 11. Snap 모드 알고리즘 상세 — MonoPix의 핵심 차별점

Snap 모드는 AI 생성 "가짜 픽셀 아트"를 **진짜 픽셀 아트**로 복원하는 MonoPix의 가장 강력한 기능입니다. AI 이미지 생성기(Midjourney, DALL-E, Stable Diffusion 등)가 만든 픽셀 아트는 겉보기에만 픽셀 아트이고, 실제로는 안티앨리어싱된 블러 경계, 불균일한 셀 크기, 서브픽셀 컬러 혼합으로 가득합니다. Snap은 이 그리드를 자동으로 역추적하여 모든 셀이 정확히 같은 크기, 단일 색상인 진짜 픽셀 아트로 재구성합니다.

**투명도 완전 보존**: RGBA 4채널 전체를 투표 키로 사용하므로, 반투명 경계나 투명 배경이 있는 AI 픽셀 아트도 깔끔하게 처리됩니다.

**해상도 자동 감지**: 사용자가 해상도를 지정할 필요 없이, 이미지 분석을 통해 원본 그리드 크기를 자동으로 추정합니다.

Snap 알고리즘은 [`fast-pixelizer`](https://github.com/handsupmin/fast-pixelizer) 패키지의 `snap()` 함수로 구현되어 있습니다.

### Color Variety 설정

사용자가 8~256 범위로 K-means 클러스터 수를 조절할 수 있습니다. 값이 클수록 원본 색상을 더 세밀하게 보존하고, 값이 작을수록 단순화된 팔레트를 생성합니다. 기본값은 256 — 최대한 원본 색상을 보존하여 가장 정확한 스냅 결과를 얻습니다.
...
**왜 사용자 설정인가**: k 값에 따라 결과물의 색상 다양성이 크게 달라짐. k=8은 극도로 단순화된 팔레트, k=256은 원본에 가장 가까운 색상 보존. 기본값 256은 최대 정확도를 우선하되, 의도적으로 팔레트를 줄이고 싶은 경우 낮은 값을 선택할 수 있도록 노출.
...
| 단계             | 시간 복잡도              | 비고                            |
| ---------------- | ------------------------ | ------------------------------- |
| K-means++ 초기화 | O(k × min(n, 50k))       | k=8~256 (사용자 설정, 기본 256) |
